...
int i,j,k,from,to, id, nproc, method=-1;
bool loop=true;
////////////////////////////////////////////////////////////////////////////
class Timer
{
...
point* update_positions_Verlet(MPI_Datatype point_type)
{
    point *aux = new point[NBALLS];
...
point* update_positions_EulerCromerMethod(MPI_Datatype point_type)
{
    point *aux = new point[NBALLS];
    ...
    for(i=from;i< to; i++ )
    {
        aux[i].vx = positions[i].vx + timeIncrement*positions[i].x*timeIncrement;
        aux[i].vy = positions[i].vy + timeIncrement*positions[i].y*timeIncrement;

        aux[i].x = positions[i].x + timeIncrement*aux[i].vx*timeIncrement;
        aux[i].y = positions[i].y + timeIncrement*aux[i].vy*timeIncrement;     
...
point* update_positions_EulerMethod(MPI_Datatype point_type)
{
    ...
    for(i=from;i< to; i++ )
    {
        ...
        aux[i].x = positions[i].x + timeIncrement*positions[i].vx*timeIncrement;
        aux[i].y = positions[i].y + timeIncrement*positions[i].vy*timeIncrement;     
...
int main(int argc, char** argv)
{
    ...
    while (loop)
    {
        ...
        if(method==1) positions = update_positions_Verlet(point_type);
        else if (method==2) positions = update_positions_EulerMethod(point_type);
        else if (method==3) positions = update_positions_EulerCromerMethod(point_type);
        else positions = update_positions_Verlet(point_type);     
        count+=1;
